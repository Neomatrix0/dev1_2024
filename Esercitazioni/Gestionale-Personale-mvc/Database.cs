...
class Database
{
    ...
    public Database()
    {
      ...
      var command1 = new SQLiteCommand(
        "CREATE TABLE IF NOT EXISTS dipendente (id INTEGER PRIMARY KEY, nome TEXT, cognome TEXT, dataDiNascita TEXT, mail TEXT) ;", 
        _connection);
    ...
    {
        var command = new SQLiteCommand($"INSERT INTO dipendente (nome,cognome) VALUES (@nome,@cognome)", _connection); // Creazione di un comando per inserire un nuovo utente
    ...
    {
        var command = new SQLiteCommand("SELECT nome,cognome FROM dipendente", _connection); // Creazione di un comando per leggere gli utenti
        ...
        while (reader.Read())
        {
            string anagrafica = reader.GetString(0) + " " + reader.GetString(1); // Aggiunta del nome dell'utente alla lista
